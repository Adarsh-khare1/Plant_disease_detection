...
def test_quality_checks_are_good(client):
    image_id = _upload_image(client)
    checks = client.post(f"/api/v1/images/{image_id}/quality").json()["checks"]
...
def test_quality_is_deterministic(client):
    image_id = _upload_image(client)
    r1 = client.post(f"/api/v1/images/{image_id}/quality").json()
    r2 = client.post(f"/api/v1/images/{image_id}/quality").json()
    assert r1 == r2
...
def test_quality_missing_image_returns_404(client):
    resp = client.post("/api/v1/images/nonexistent_id/quality")
